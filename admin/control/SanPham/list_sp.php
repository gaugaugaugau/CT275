<div class="col-sm-9" id="content" >
	...
	<table class="table table-bordered" id="table_List_SP">
		...
		<?php 
			//Phan trang
			$nb_sanpham = !empty($_GET['per_page'])?$_GET['per_page']:"10";
			$nb_trang = !empty($_GET['page'])?$_GET['page']:"1";
			$offset = ($nb_trang-1) * $nb_sanpham;
			$sum_sanpham = mysqli_query($connect,"SELECT * from hanghoa");
			$sum_sanpham = $sum_sanpham->num_rows;
			$sum_trang = ceil($sum_sanpham / $nb_sanpham);
			//echo $sum_trang;
			// echo $sum_sanpham;
			//_____________
			...
			$sql = "SELECT * FROM hanghoa h JOIN nhomhanghoa n 
					...
					JOIN danhmuc d on
					n.MaDanhMuc = d.MaDanhMuc order by h.MSHH desc
					limit $nb_sanpham offset $offset ";
